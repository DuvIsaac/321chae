...
class Game:
    ...
    def draw_optimization(self, screen):
        ...
        screen.blit(optimization_text, (WIDTH//2 + 10, 10))

        optimized_score_text = self.font.render(f"Optimized Score: {self.optimized_score}", True, BLACK)
        screen.blit(optimized_score_text, (WIDTH//2 + 10, 50))
        ...
        while running:
            for event in pygame.event.get():
                if event.type == pygame.QUIT:
                    ...
                    if x < WIDTH // 2:  # Only process clicks on the left side (game area)
