...
class Game:
    ...
    def draw_buttons(self):
        ...
        for color, rect in TRAIN_BUTTONS.items():
            # 개선된 기차 버튼 디자인
            train_color = color if self.lines[color].train else (200, 200, 200)
            pygame.draw.rect(screen, train_color, rect)
            pygame.draw.circle(screen, BLACK, (rect.x + BUTTON_WIDTH // 2, rect.y + BUTTON_HEIGHT // 2), BUTTON_HEIGHT // 3)
            pygame.draw.circle(screen, WHITE, (rect.x + BUTTON_WIDTH // 2, rect.y + BUTTON_HEIGHT // 2), BUTTON_HEIGHT // 3 - 2)
